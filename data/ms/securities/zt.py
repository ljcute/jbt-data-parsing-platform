...
def _format_dbq(cdata, market):
    ...
    jzd['rate'] = jzd['rate'].fillna('空')
    jzd['rate'] = jzd['rate'].apply(lambda x: 1 if str(x) == 'A' else 2 if str(x) == 'B' else 3 if str(x) == 'C' else 4 if str(x) == 'D' else 5 if str(x) == 'E' else 6 if str(x) == 'F' else None if str(x) == '空' else str(x))
