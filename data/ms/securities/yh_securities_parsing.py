...
# @Site    : 
# @File    : yh_securities_parsing.py
# @Software: PyCharm
from data.ms.genralhandler import *


def yh_parsing_data(rs, data_):
    bzj_data = []
    rz_data = []
    rq_data = []
    error_list = []
    if rs[2] == '2':
        logger.info(f'中国银河证券可充抵保证金证券解析开始...')
        for data in data_:
            sec_code = data[0]
            sec_name = data[1]
            rate = data[2]
        ...
        logger.info(f'中国银河证券可充抵保证金证券解析结束...')

    elif rs[2] == '4':
        logger.info(f'中国银河证券融资标的证券解析开始...')
        for data in data_:
            sec_code = data[0]
            sec_name = data[1]
            rate = data[2]
        ...
        logger.info(f'中国银河证券融资标的证券解析结束...')

    elif rs[2] == '5':
        logger.info(f'中国银河证券融券标的证券解析开始...')
        for data in data_:
            sec_code = data[0]
            sec_name = data[1]
            rate = data[2]
        ...
        logger.info(f'中国银河证券融券标的证券解析结束...')
